...
// [[Rcpp::export]]
Rcpp::NumericVector rcpp_A_OPT(NumericMatrix res,NumericMatrix inv_v) { //page 19 in article
	int n =res.nrow();
	int m = res.ncol();
	double* res_c = R2C_mat(res);
	...
	for (long i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			for (int k = 0; k < m; k++) {
				res_inv[i*m + j] += res_c[i*m + k]*inv_v_c[j*m + k]; //cheked manualy
			}
		}
	}
...
// [[Rcpp::export]]
Rcpp::NumericVector rcpp_B_OPT(NumericMatrix res, NumericVector inv_diag_v) { //my addition
	int n = res.nrow();
	int m = res.ncol();
	double* res_c = R2C_mat(res);
	double* inv_v_diag_c = R2C_vec(inv_diag_v);
	double* res_inv = (double*)malloc(sizeof(double) * n * m);
	memset(res_inv, 0, sizeof(double) * n * m);
	for (long i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			res_inv[i * m + j] = res_c[i * m + j] * inv_v_diag_c[j];
		}
	}
...
// [[Rcpp::export]]
Rcpp::List rcpp_score_wrapper(NumericVector beta, NumericVector weights,NumericVector times,NumericVector truncs, NumericVector status, NumericMatrix covariates,int hess,int residuals, int diag) {
	...
	double* ret_diag = NULL; //new
	if (residuals) {
		ret_residual = (double*)malloc(sizeof(double)*c * r);
		memset(ret_residual,0,sizeof(double)*c * r); // what is this doing exactly??
	}
	if (hess) {
	...
	if (diag) {
		ret_diag = (double*)malloc(sizeof(double) * r);
		memset(ret_diag, 0, sizeof(double) * r); //new
	}
	double* T = (double*)malloc(sizeof(double)*2 * n); //WHAT HAPPANES HERE?
	int* ord = (int*)malloc(sizeof(int)*2 * n);
	memcpy(T,times_c,sizeof(double)*(n));
	if (use_truncs) {
		memcpy(T+n,truncs_c,sizeof(double)*(n)); //WHAT IS THIS
	} else {
		memset(T+n,0,sizeof(double)*(n));
	}
	for (int i = 0; i< 2*n; i++) { //WHAT IS THIS FOR
		ord[i] = i;
	}
	less_than_key cmp(T,D,n); //WHAT HAPPENS HERE
	//sort(ord,ord + (2*n), [&](int i,int j){return T[i]<T[j];} );
	sort(ord,ord + (2*n), cmp);
	double S0 = 0;
	double part_a = 0; //What are part a and b?
	...
	double S1[r];
	double S2[r][r];
	double S2_diag[r]; //new
	double part_b[r];
	memset(S1,0, sizeof(double)*r);
	memset(S2,0, sizeof(double)*r*r);
  memset(S2_diag,0, sizeof(double)*r); //new
	memset(part_b,0, sizeof(double)*r);

	long found_c = 0;
	long i = 0;
	for (long o=0;o<n*2;o++) { //don't understand what's going on in this loop
		i = ord[o];
		if (i >= n) {
			long ip = i - n; // I don't understnad what's happening here
			...
			for (int j = 0; j<r;j++) {
				...
				if (diag) {
					S2_diag[j] += expbw * X[ip * r + j] * X[ip * r + j];
				} //new
			}
		}
		else {
			double cur_T  = T[i]; //What is this
			long end_o = o;
			while (T[i] == cur_T && i < n && end_o < n*2) { //and here
				if (D[i]) {
					if (hess) {
						for (int j = 0; j<r;j++) {
							for (int k = j; k<r;k++) {
					...
					if (diag) {
						for (int j = 0; j < r; j++) {
							ret_diag[j] -= ((S2_diag[j] / S0) - (S1[j] / S0) * (S1[j] / S0));
						}
					} //new
					if (residuals) {		
						part_a += (1/S0);
			...
			for (;o < end_o; o++) {
				...
				for (int j = 0; j<r;j++) {
					...
					if (diag) {
						S2_diag[j] -= expbw * X[i * r + j] * X[i * r + j];
					} //new
	...
	return Rcpp::List::create(Rcpp::Named("residual") = out_residuals,
                          Rcpp::Named("hess") = out_hess,Rcpp::Named("ord") = out_ord, Rcpp::Named("diagon") = out_diag);
}
